...
describe('Starting tests for angular-app', function() {
  ...
  it('network-name should be blood@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('blood@0.0.1.bna');
    });
  });
  ...
  it('navbar-brand should be angular-app',() => {
    ...
    it('Source component should be loadable',() => {
      page.navigateTo('/Source');
      browser.findElement(by.id('participantName'))
      .then((participantName) => {
        return participantName.getText();
      })
      .then((txt) => {
        expect(txt).toBe('Source');
      });
    });

    it('Source table should have 5 columns',() => {
      page.navigateTo('/Source');
      element.all(by.css('.thead-cols th')).then(function(arr) {
        expect(arr.length).toEqual(5); // Addition of 1 for 'Action' column
      });
    ...
    it('Laboratory component should be loadable',() => {
      page.navigateTo('/Laboratory');
      browser.findElement(by.id('participantName'))
      .then((participantName) => {
        return participantName.getText();
      })
      .then((txt) => {
        expect(txt).toBe('Laboratory');
      });
    });

    it('Laboratory table should have 5 columns',() => {
      page.navigateTo('/Laboratory');
      element.all(by.css('.thead-cols th')).then(function(arr) {
        expect(arr.length).toEqual(5); // Addition of 1 for 'Action' column
      });
    ...
    it('BloodBank component should be loadable',() => {
      page.navigateTo('/BloodBank');
      browser.findElement(by.id('participantName'))
      .then((participantName) => {
        return participantName.getText();
      })
      .then((txt) => {
        expect(txt).toBe('BloodBank');
      });
    });

    it('BloodBank table should have 5 columns',() => {
      page.navigateTo('/BloodBank');
      element.all(by.css('.thead-cols th')).then(function(arr) {
        expect(arr.length).toEqual(5); // Addition of 1 for 'Action' column
      });
